...
class IDSPipeline(StableDiffusionPipeline):

    def progress_bar(self, iterable=None, total=None, position=None):
        ...
        if iterable is not None:
            ...
            if position is not None and position > 0:
                return tqdm(total=total, position=position, leave=False, **self._progress_bar_config)
            else:
    ...
    def __call__(
            ...
            w_dds: float = 1.0,
            w_cut: float = 3.0, # if w_cut > 0, use CDS loss

            # Additional args for IDS
            scale: float = 1.0, # lambda in paper
            iter_fp: int = 3,
            position: int = 0,
            save_path: str = None,
            with_cut: bool = False,
        ...
        # use CUT loss for IDS (IDS can be utilized with any SDS-based methods)
        if with_cut:
        ...
        with self.progress_bar(total=num_inference_steps, position=position) as progress_bar:
            for i in range(num_inference_steps):

                optimizer.zero_grad()

                # FPR Loss
                # get z_t_src by forward process [Eq. (1)]
                z_t_src, eps, timestep = ids_loss.noise_input(z_src, eps=None, timestep=None)

                # to obatin z_t^(src*) using FPR [Eq. (6) & Eq. (7)]
                # line 4~9 in Algo. 1
                z_t_src, norm = ids_loss.update_zt(
                    z_src,
                    z_t_src,
                    timestep,
                    prompt_embeds,
                ...
                # get epsilon* by inverse of forward process [Eq. (8)]
                eps = ids_loss.estimate_eps(z_src, z_t_src, timestep)

                # calculate z_t_trg by forward process with shared epsilon
                z_t_trg, _, _ = ids_loss.noise_input(z_trg, eps, timestep)

                # get score for dds & reference attention maps
                eps_pred = ids_loss.get_epsilon_prediction(
                ...
                if with_cut:
                    ...
                    with torch.enable_grad():
                        z_t_trg, _, _ = ids_loss.noise_input(z_trg, eps, timestep)
                        eps_pred_trg = ids_loss.get_epsilon_prediction(
                                z_t_trg,
                                timestep,
                                trg_prompt_embeds,
                            )
                ...
                # save intermediate result during translation
                if (save_path is not None) and (i % 50 == 49):
                    result_tmp = self.decode_latents(z_trg).squeeze()
                    result_tmp = Image.fromarray((result_tmp * 255).astype(np.uint8))
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    result_tmp.save(os.path.join(save_path, f"{str(i).zfill(3)}.png"))
